...
LABEL_SEG_CODE32:
	...
	push	ProcHUST         ; # 页起始地址 Destination
	call	MemCpy
	add	esp, 12
	...
	; # 而当两个任务都是死循环时，会发生任务0未执行完而发生中断去执行任务1，则最终会跳回来继续执行任务0
	; # 而由于任务1也是死循环，会导致该任务也没有执行完，中断发生后跳进任务0，最终又会跳回来继续执行任务1
...
.5:
	...
	mov	dword [es:eax], ProcLYG | PG_P | PG_USU | PG_RWW
